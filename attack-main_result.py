...
parser.add_argument('--method', type=str, default='DITIMI')
parser.add_argument('--model', type=str, default='inceptionv4')
...
def Spectrum_Simulation_Attack(images, gt, model, min, max, min1, max1):
    """
    ...
    for i in range(num_iter):
        ...
        if i > num_iter // 2:
            ...
            for k in range(50):
                ...
                loss = F.cross_entropy(output_up_x, gt)
                loss.backward()
                up_x = up_x  - alpha/2 * torch.sign(up_x.grad.data)
                up_x = clip_by_tensor(up_x, min1, max1)
                model.zero_grad()
...
def main():
    ...
    if opt.model == 'inceptionresnetv2':
        model = torch.nn.Sequential(Normalize(opt.mean, opt.std),
    ...
    X = ImageNet(opt.input_dir, opt.input_csv, transforms, num_images=opt.num_images)
    ...
    for images, images_ID,  gt_cpu in tqdm(data_loader):
        ...
        min1 = clip_by_tensor(images - opt.max_epsilon/2 / 255.0, 0.0, 1.0)
        max1 = clip_by_tensor(images + opt.max_epsilon/2 / 255.0, 0.0, 1.0)

        adv_img = Spectrum_Simulation_Attack(images, gt, model, images_min, images_max, min1, max1)
